...
// var cm_dashboard *ConnectionManager = nil
// var cm_api *ConnectionManager = nil
...
func (this *ConnectionManager) getServerList(project string) ([]string, error) {
	...
	default:
		if project != "auth" {
			...
			if result, err, _, _, _ := this.ExecuteSQLArray("auth", query, &args); result == nil || result.GetNumberOfRows() == 0 {
...
// Executes the query on one of the servers that belong to the project
// The first return value is the result, it can be nil in case of error.
// The second return value is the error generated by the sdk library, if any
// The third return value is the sqlite result code
// The fourth retrun value is the sqlite extended result code
// The fifth return value is the byte offset of the most recent error
func (this *ConnectionManager) ExecuteSQL(project string, query string) (*sqlitecloud.Result, error, int, int, int) {
	return this.ExecuteSQLArray(project, query, nil)
}

// Executes the query on one of the servers that belong to the project.
// Any placeholder in the query will be binded with the corresponding object in the args slice.
// The first return value is the result, it can be nil in case of error.
// The second return value is the error generated by the sdk library, if any
// The third return value is the sqlite result code
// The fourth retrun value is the sqlite extended result code
// The fifth return value is the byte offset of the most recent error
func (this *ConnectionManager) ExecuteSQLArray(project string, query string, args *[]interface{}) (*sqlitecloud.Result, error, int, int, int) {
	...
	for try := 0; try < maxTries; try++ {
		...
		default:
			...
			start := time.Now()
			errCode := int(0)
			extErrCode := int(sqlitecloud.NO_EXTCODE)
			errOffset := int(sqlitecloud.NO_OFFCODE)
