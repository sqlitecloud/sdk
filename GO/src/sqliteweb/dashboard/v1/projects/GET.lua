...
if userID == 0 then                                           -- get list of projects in ini file
  ...
  data = executeSQL( "auth", string.format( "SELECT uuid AS id, Project.name, description FROM User JOIN Company ON User.company_id = Company.id JOIN Project ON Company.id = Project.company_id WHERE User.enabled = 1 AND user.id = %d;", userid ) )
