...
if not body.name                        then return error( 400, "Missing new role name" ) end
if string.len( body.name )    < 1       then return error( 400, "Invalid new role name" ) end

query = string.format( "RENAME ROLE '%s' TO '%s'; ", enquoteSQL( roleName ), enquoteSQL( body.name ) )
