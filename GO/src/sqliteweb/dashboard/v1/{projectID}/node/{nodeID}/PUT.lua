...
local name,      err, msg = getBodyValue( "name", 0 )                    if err ~= 0 then return error( err, msg )                          end -- Dev1 Server
local type,      err, msg = getBodyValue( "type", 0 )                    if err ~= 0 then return error( err, msg )                          end -- worker
local provider,  err, msg = getBodyValue( "provider", 0 )                if err ~= 0 then return error( err, msg )                          end -- DigitalOcean
local image,     err, msg = getBodyValue( "image", 0 )                   if err ~= 0 then return error( err, msg )                          end -- i386/1/1MB/100MB
local region,    err, msg = getBodyValue( "region", 0 )                  if err ~= 0 then return error( err, msg )                          end -- Rome/Italy
local size,      err, msg = getBodyValue( "size", 0 )                    if err ~= 0 then return error( err, msg )                          end -- small
local address,   err, msg = getBodyValue( "address", 0 )                 if err ~= 0 then return error( err, msg )                          end -- 64.227.11.116
local port,      err, msg = getBodyValue( "port", 0 )                    if err ~= 0 then return error( err, msg )                          end -- 9960
